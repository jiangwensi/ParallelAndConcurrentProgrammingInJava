 ...
 */
public final class StudentAnalyticsBk {
    /**
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        List<Student> inactiveStudents = Stream.of(studentArray)
                .parallel()
                .filter(s -> !s.checkIsCurrent()).collect(Collectors.toList());

//        Map<String, Integer> nameCounts = Collections.synchronizedMap(new HashMap<>());
        Map<String, Integer> nameCounts = new HashMap<>();

        inactiveStudents.stream()
                .forEach(e -> {
                    if (nameCounts.containsKey(e.getFirstName())) {
                        nameCounts.put(e.getFirstName(), new Integer(nameCounts.get(e.getFirstName()) + 1));
                    } else {
                        nameCounts.put(e.getFirstName(), 1);
                    }
                });
