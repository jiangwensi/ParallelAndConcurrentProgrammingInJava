...
//        boolean collapsedToSingleNode = false;
        ParComponent resultNode=null;
        while ((loopNode = nodesLoaded.poll()) != null) {
            /*
             * poll() removes first element (node loopNode) from the nodesLoaded
             * work-list.
             */
            if (!loopNode.lock.tryLock()) {//some other thread is working on this node. continue;
                continue;
            }
            ...
            if (e == null) {
//                loopNode.lock.unlock();
//                collapsedToSingleNode=true;
                resultNode = loopNode;
                break; // done - we've contracted the graph to a single node
            }

            final ParComponent other = e.getOther(loopNode);//some other thread is working on this node. continue;
    ...
    public static final class ParComponent extends Component<ParComponent> {
        ...
        private ParComponent solution = null;

        public ParComponent getSolution() {
            return solution;
        }

        public void setSolution(ParComponent solution) {
